...
# Constants for Radial dB Meters
# -----------------------------
DIAMOND_POWER = 80          # Multiplier for diamond separation
DIAMOND_SIZE = 35           # Half-width/height of the diamond triangles
DRAW_SMALL_TRIANGLES = True  # Draw the original small triangles if desired
TRIANGLE_1_DISTANCE = 55
TRIANGLE_1_SIZE = 25
NUM_SIDES = 30              # For drawing circle outlines
BOUNCE_INTENSITY = 10
INNER_CIRCLE_RADIUS = 180
OUTLINE_THICKNESS = 4
ANGLE_INCREMENT = 12        # degrees between each radial bar

# New constants for the second smaller triangle:
DRAW_SECOND_SMALL_TRIANGLE = True
SECOND_SMALL_TRIANGLE_SIZE = 15    # Half-size of the second small triangle
SECOND_SMALL_TRIANGLE_OFFSET = 120    # Vertical offset from center for the second small triangle
...
# -----------------------------
# Pygame version of draw_separated_diamond
# -----------------------------
def draw_separated_diamond(center_x, center_y, glow_value):
    # Compute separation based on glow value and a constant multiplier.
    separation = glow_value * DIAMOND_POWER
    # Calculate diamond color using manual_hue_value, control_sensitivity and control_brightness_floor.
    r, g, b = colorsys.hsv_to_rgb(manual_hue_value, 1, max(glow_value * control_sensitivity, control_brightness_floor))
    ...
    def rotate_point(x, y, cx, cy):
        # Rotate (x, y) about center (cx, cy) by 45°.
        x, y, cx, cy = float(x), float(y), float(cx), float(cy)
        x -= cx
        y -= cy
    ...
    # Define the four main triangles of the diamond.
    triangle_vertices = [
    ...
    for triangle in triangle_vertices:
        rotated_triangle = [rotate_point(pt[0], pt[1], center_x, center_y) for pt in triangle]
        pygame.draw.polygon(screen, diamond_color, rotated_triangle)

    # Optionally draw the original small triangles.
    if DRAW_SMALL_TRIANGLES:
        small_triangle_1_vertices = [
            [(center_x, center_y + separation + TRIANGLE_1_DISTANCE),
             (center_x - TRIANGLE_1_SIZE, center_y + TRIANGLE_1_SIZE + separation + TRIANGLE_1_DISTANCE),
             (center_x + TRIANGLE_1_SIZE, center_y + TRIANGLE_1_SIZE + separation + TRIANGLE_1_DISTANCE)],
            [(center_x + separation + TRIANGLE_1_DISTANCE, center_y),
             (center_x + TRIANGLE_1_SIZE + separation + TRIANGLE_1_DISTANCE, center_y + TRIANGLE_1_SIZE),
             (center_x + TRIANGLE_1_SIZE + separation + TRIANGLE_1_DISTANCE, center_y - TRIANGLE_1_SIZE)],
            [(center_x, center_y - separation - TRIANGLE_1_DISTANCE),
             (center_x - TRIANGLE_1_SIZE, center_y - TRIANGLE_1_SIZE - separation - TRIANGLE_1_DISTANCE),
             (center_x + TRIANGLE_1_SIZE, center_y - TRIANGLE_1_SIZE - separation - TRIANGLE_1_DISTANCE)],
        ...
        for small_triangle in small_triangle_1_vertices:
            rotated_small_triangle = [rotate_point(pt[0], pt[1], center_x, center_y) for pt in small_triangle]
            pygame.draw.polygon(screen, diamond_color, rotated_small_triangle)

    # NEW: Draw the second, smaller triangle following the same rotation logic.
    if DRAW_SECOND_SMALL_TRIANGLE:
        # Compute the apex of the second triangle.
        # For example, we can position its apex above the center:
        apex = (center_x, center_y - separation - SECOND_SMALL_TRIANGLE_OFFSET)
        # The base vertices will be offset horizontally by SECOND_SMALL_TRIANGLE_SIZE
        base_left = (center_x - SECOND_SMALL_TRIANGLE_SIZE, center_y - separation - SECOND_SMALL_TRIANGLE_OFFSET + SECOND_SMALL_TRIANGLE_SIZE)
        base_right = (center_x + SECOND_SMALL_TRIANGLE_SIZE, center_y - separation - SECOND_SMALL_TRIANGLE_OFFSET + SECOND_SMALL_TRIANGLE_SIZE)
        second_triangle_vertices = [apex, base_left, base_right]
        # Optionally rotate these vertices around the center for consistency.
        rotated_second_triangle = [rotate_point(pt[0], pt[1], center_x, center_y) for pt in second_triangle_vertices]
        pygame.draw.polygon(screen, diamond_color, rotated_second_triangle)


# -----------------------------
# Pygame version of draw_circle_outline
# -----------------------------
def draw_circle_outline(center_x, center_y, radius, outline_thickness, outline_color):
    ...
    for i in range(NUM_SIDES):
        ...
        points.append((x, y))
    pygame.draw.polygon(screen, outline_color, points, outline_thickness)



# -----------------------------
# Pygame version of draw_radial_db_meters
# -----------------------------
def draw_radial_db_meters():
    global latest_audio_data, glow_value, manual_hue_value, control_sensitivity, control_brightness_floor
    try:
        if latest_audio_data is None or len(latest_audio_data) < 2:
            return
        ...
        max_bar_length = DEFAULT_MAX_BAR_LENGTH
        base_radius = DEFAULT_RADIUS
        ...
        dynamic_radius = float(base_radius + glow_value * BOUNCE_INTENSITY)
        dynamic_bar_length = float(max_bar_length + glow_value * BOUNCE_INTENSITY)

        center_x = window_width // 2
        center_y = window_height // 2

        circle_radius = float(INNER_CIRCLE_RADIUS + glow_value * BOUNCE_INTENSITY)
        or_r, or_g, or_b = colorsys.hsv_to_rgb(manual_hue_value, 1, max(glow_value * control_sensitivity, control_brightness_floor))
        circle_outline_color = (int(or_r * 255), int(or_g * 255), int(or_b * 255))
        draw_circle_outline(center_x, center_y, circle_radius, OUTLINE_THICKNESS, circle_outline_color)
        ...
        for i in range(num_bars):
            ...
            bar_length = bar_amplitudes[i] * dynamic_bar_length if db_value >= -50 else 0

            start_x = int(center_x + dynamic_radius * math.cos(angle))
            start_y = int(center_y + dynamic_radius * math.sin(angle))
            end_x = int(start_x + bar_length * math.cos(angle))
            end_y = int(start_y + bar_length * math.sin(angle))
